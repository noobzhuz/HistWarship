        ...
        <>
          ...
          {matchingSites.map((site) => onSelectSite ? (
            <div key={site.id} className="flex items-start gap-3 rounded-2xl p-3 hover:bg-slate-100">
              ...
              <span className="min-w-0 flex-1"><Link href={`/sites/${site.slug}`} className="block font-bold hover:text-sky-800 hover:underline">{site.name}</Link><span className="text-sm text-slate-500">{site.city}, {site.country}</span></span>
